...
{
  if(in_array('name',$optin_datas['extracols'])){echo "<th>Name</th>";};
  if(in_array('email',$optin_datas['extracols'])){echo "<th>Email</th>";};
...
?>
<th>IP</th><th>Page</th><th>Added On</th><th>Action</th></thead>
...
while($r=$optin_datas['leads']->fetch_object())
{
	...
	$action="<table class='actionedittable'><tr><td><form action='' method='post'><input type='hidden' name='deleteoptin' value='".$r->id."'><button type='submit' class='btn unstyled-button' data-toggle='tooltip' title='Delete Record'><i class='fas fa-trash text-danger'></i></button></form></td></tr></table>";
...
if($optin_datas['extracols'] !=0)
{
  if(in_array('name',$optin_datas['extracols'])){echo "<td>".$r->name."</td>";};
  if(in_array('email',$optin_datas['extracols'])){echo "<td>".$r->email."</td>";};
	for($i=0;$i<count($optin_datas['extracols']);$i++)
	{
		if(in_array($optin_datas['extracols'][$i],array('name','email','password','reenterpassword')))
		{
  ...
  echo "<td>".$r->ipaddr."</td><td>".$url."</td><td>".date('d-M-Y h:ia',$r->addedon)."</td><td>".$action."</td></tr>";
}
?>
<tr><td colspan=1000000 class="total-data">Total Optins: <?php echo number_format($optin_datas['total']); ?></td></tr>
<!-- /keyword search -->
</tbody>
<?php
 }
...
<form action="index.php?page=export_csv" method="post"><button type="submit" class="btn theme-button" style="float:right;" name="optinto_csv" value="<?php if(isset($_GET['funnelid'])){ echo $_GET['funnelid']; } ?>"><i class="fas fa-file-download"></i> Export To CSV</button></form>
</div>
</div>
