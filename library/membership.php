...
class Membershipqfnl
{
	...
	function hiddenAccountCreator($regpageid,$name,$email,$datas)
	{
		...
		$qry=$mysqli->query("select `funnelid`,`title`,`filename` from `".$page_table."` where `category`='login'");
	...
	function createMember($funnelid,$pageid,$name,$email,$password,$exf,$verifycode,$verify=1,$update=0,$autocreate=0,$multiaccount=0)
	{
		...
		$verifycode=$mysqli->real_escape_string($verifycode);
		  ...
		  foreach($edatas as $index=>$edata)
		  {
			  ...
			  $edatas[$index]=$edata;
			  if($index=='submit'){unset($edatas[$index]);continue;}
			  elseif($index=='name')
			  {
				  unset($edatas[$index]);
			  }
		  ...
		  $verified=0;
			...
			else
			{
				...
				$password=password_hash($password,PASSWORD_DEFAULT);
				$in=$mysqli->query("insert into `".$table."` (`funnelid`, `pageid`, `name`, `email`, `password`, `verified`, `verifycode`, `date_verifycodegen`, `ip_created`, `ip_lastsignin`, `date_created`, `date_lastsignin`,`valid`, `exf`) values ('".$funnelid."','".$pageid."','".$name."','".$email."','".$password."','1','".$verifycode."','','".$this->ip."','N/A','".time()."','N/A','1','".$exfjsn."')");
			...
			else{return get_option('un_auth_access_err');/*not verified*/ }
			 }
	...
	function memberLogin($funnelid,$pageid,$email,$password,$cookieuser=0)
	{
		...
		if($page=="login")
		{
			...
			if($qry->num_rows>0)
			{
				...
				// $chk_qry="select `id`,`productid` from `".$sale_table."` where `membership` like '".$data->id.",%' or `membership` like '%,".$data->id.",%' or `membership` like '%,".$data->id."' and `payment_id`='".$exfs['payment_id']."'";
	...
	{
		$reg="/(@-cf-veried-){1}(\d)+(-member-@){1}/";
		$content=preg_replace_callback($reg,function($chk){
			if(isset($chk[0]))
			{
				$chk[0]=str_replace("@-cf-veried-","",$chk[0]);
	...
	function createMembeshipRegLink($funnelid,$pageid=0,$url)
	{
		...
		{
			//`funnelid`, `pageid`, `name`, `email`, `password`, `verified`, `verifycode`, `date_verifycodegen`, `ip_created`, `ip_lastsignin`, `date_created`, `date_lastsignin`,`valid`, `exf`
			$in=$mysqli->query("insert into `".$table."` (`funnelid`,`pageid`,`name`, `email`, `password`, `verified`, `verifycode`, `date_verifycodegen`, `ip_created`, `ip_lastsignin`, `date_created`, `date_lastsignin`,`valid`,`exf`) values ('".$funnelid."','".$pageid."','','','','0','".$otp."','','','','','','1','')");
		}
	...
	function isMembershipPage($pageid)
	{
		...
		$pageid=(int)$pageid;
		$reg_pages=array();$login_pages=array();$memberarea_pages=array();$forgotpassword_pages=array();
	...
	function visualOptisForFunnels($funnel_id='all',$pagecount=0,$search="")
	{
		...
		$countsql="select count(`id`) as totaloptins from `".$table."` where `email` not in('',' ')".$datebetween;
		...
		{
		$countsql="select count(`id`) as totaloptins from `".$table."` where funnelid='".$funnel_id."' and `email` not in ('',' ')".$datebetween;
		...
		if($pagecount==0)
		{
		$sql="select * from `".$table."` where funnelid='".$funnel_id."' and `email` not in('',' ')".$datebetween." order by ".$order_by." limit ".get_option('qfnl_max_records_per_page')."";
		}
		else
		{
		$pagecount=($pagecount*get_option('qfnl_max_records_per_page'))-get_option('qfnl_max_records_per_page');
		$sql="select * from `".$table."` where funnelid='".$funnel_id."' and `email` not in('',' ')".$datebetween." order by ".$order_by." limit ".$pagecount.", ".get_option('qfnl_max_records_per_page')."";
		}
		...
		$sql="select * from `".$table."` where funnelid='".$funnel_id."' and (name like '%".$search."%' or email like '%".$search."%' or exf like '%:\"%".$search."%\"%' or `ip_created` like '%".$search."%' or `ip_lastsignin` like '%".$search."%') and `email` not in('',' ') order by id desc";
		}
	     ...
	     while($r=$getpagedataqry->fetch_object())
			...
			if($optinqry->num_rows>0)
			{
				...
				while($r=$optinqry->fetch_assoc())
				{
					...
					{
						//array('Register_IP','Last_Assigned_IP','Added_On','Last_Activity')
						if($csvheaders[$i]=="Register_IP"){$csvheaders[$i]="ip_created";}
						if($csvheaders[$i]=="Last_Assigned_IP"){$csvheaders[$i]="ip_lastsignin";}
						if($csvheaders[$i]=="Added_On"){$csvheaders[$i]="date_created";}
						if($csvheaders[$i]=="Last_Activity"){$csvheaders[$i]="date_lastsignin";}
