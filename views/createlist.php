...
<div class="container" id="createlist">
		...
		<div class="row">
			...
			<div class="col-sm-4 col-sm-offset-4">
				...
				<div class="form-group">
					...
					<input type="text" v-model="quick_list_name" class="form-control" required v-bind:placeholder="t('Enter List Name')"></div>
	...
	<?php } else if (is_numeric($_GET['listid'])) {
		...
		?>
		<div class="form-group" id="editorform">
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-6">
								...
								<tbody id="loadinglistqml">
									<?php
										while ($row = $result->fetch_assoc()) {
											...
											$action = "<button style='margin-right:10px;' class='btn unstyled-button' data-toggle='tooltip' title='".t('Edit')."' onclick='editRecord(" . $row['id'] . ")'><i class='text-primary fas fa-edit'></i></button><form action='' method='post' style='display:inline;'><button class='btn unstyled-button' name='listdeleterec' value=" . $row['id'] . " data-toggle='tooltip' title='".t('Delete Record')."'><i class='text-danger fas fa-trash'></i></span></button></form>";

											echo "<tr><td>" . t($hashcount) . "</td><td id='listusername" . $row['id'] . "'>" . $row['name'] . "</td><td id='listuseremail" . $row['id'] . "'>" . $row['email'] . "</td><td>" . $row['ipaddr'] . "</td><td class='text-center'><button class='btn unstyled' " . $disabled . "  onclick=viewExfData(" . $row['id'] . ")><i class='text-secondary text-center fas fa-eye'  data-toggle='tooltip' title='View Record'></i></button></td><td>" .date('d-M-Y h:ia',$row['date']) . "</td><td class='text-right'>" . $action . "</td></tr>";
										}
										?>
								</tbody>
							</table>
...
<script type="text/javascript">
	...
	function viewExfData(id) {
		...
		request.postRequestCb('req.php', {"viewlistexfdata":id}, function(data) {
	...
	function wqmlrUploadCsv(str) {
		if (str == 'open') {
			...
			reader.onload = function() {
				...
				var selectname = "<select class='form-control'><option value='NO'>"+t("Select Name Field")+"</option>";

				var selectemail = "<select class='form-control'><option value='NO'>"+t("Select Email Field")+"</option>";
				...
				document.getElementById("wpqmlrcsvfilesave").onclick = async function() {
					...
					if (namecols[1].value != "NO") {
						...
						for (var i = 1; i < arr.length; i++) {
							...
							//alert(temparr[namecols[1].value]);
						...
						if (count > 0) {
							//console.log(JSON.stringify(jsnarr));
							...
							for (var i = 0; i < totalupload; i++) {
								try {
									...
									request.postRequestCb('req.php',{'ajxsavetolist': JSON.stringify(jsnarr[i]),'listid':listid }, function(data) {
										++totaldone;
