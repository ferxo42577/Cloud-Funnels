    ...
    methods:{
        ...
        createPluginChanger:function(id){
            if(id && (plugin_control.plugins[id] !==undefined)){
                ...
                this.plugin_detail=(plugin.u_version_detail !==undefined)? plugin.u_version_detail:'No detail available';
            }
        }
    },
    watch:{
...
Vue.component('container_plugin_installer',{
    ...
    methods:{
        closePopup:function(){plugin_control.show_plugin_installer_popup=false;},
    ...
    methods:{
        ...
        getMarketPlaceURL:function(){
            let current_url=window.location.href;
                ...
                return `http://marketplace.cloudfunnels.in/?plugin_ref_installation=${btoa(current_url)}`;
        },
        loadPlugins:function(){
            this.err="";
            this.ajax_loading=true;
            this.request.postRequestCb('req.php',{manage_plugins:1,load:'all'},(res)=>{
                ...
                }catch(err){ console.log(err); this.err=err.message; }
            });
        },
        loadUpdateChecker:function(plugin_id=false){
            ...
            for(let i in plugins)
            {
                ...
                this.plugins[i].u_stat_class=(status=='checking')? 'fa-spinner fa-spin text-primary':'fa-sync-alt text-info';
        ...
        descriptionCreate:function(des,idd){
            ...
            if(des.length>8)
            {
                ...
                let r_more= `${show.join(' ')}<span style="display:none" hdndescription="${idd}"> ${pending.join(" ")}</span><span activehiddendescription="${idd}">...<span class="plugin-readmore">Read More</span></span>`;
                setTimeout((idd)=>{
                    try
                    {
                    document.querySelectorAll(`span[activehiddendescription="${idd}"]`)[0].onclick=function(e){
                        try
                        {
                            document.querySelectorAll(`span[activehiddendescription="${idd}"]`)[0].style.display="none";
                            document.querySelectorAll(`span[hdndescription="${idd}"]`)[0].style.display="block";
                        }catch(err){console.log(err);}
                    };
                    }catch(err){console.log(err);}
                },500,idd);
        ...
        doActiveorDeactive:function(plugin_id,e){
            ...
            this.request.postRequestCb("req.php",data,(res)=>{
                ...
                if(res=='1')
                {
                    ...
                    this.plugins[plugin_id].active=(this.plugins[plugin_id].active)? false:true;
                    this.msg=this.t(`The plugin ${doo}d successfuly.`);
                    setTimeout(()=>{ window.location="";},500);
                }
                else
                {
                    this.err=this.t("The plugin generated unwanted output")+": "+this.t(res);
                }
            });
        },
        ...
        doInstallRemotePlugin:function(plugin_url){
            ...
            this.msg="";
            this.ajax_loading=true;
            this.request.postRequestCb('req.php',{manage_plugins:1,upload_remote_plugin:plugin_url},res=>{
                this.ajax_loading=false;
                res=res.trim();
                if(res==='1')
                {
        ...
        doDelete:function(plugin_id){
            this.msg="";
            this.err="";
            let cnf=confirm(this.t("Are you sure?"));
            if(!cnf){return false;}
            this.ajax_loading=true;
            this.request.postRequestCb('req.php',{manage_plugins:1,del_plugin:plugin_id},(res)=>{
                this.ajax_loading=false;
                res=res.trim();
                if(res=='1')
                {
            ...
            this.request.postRequestCb('req.php',{
                ...
                try{
                    ...
                    if(stat.update)
                    {
                        ...
                        this.plugins[plugin_id].u_stat_class="fa-cloud-download-alt text-warning";
                        this.plugins[plugin_id].u_file=stat.file;
                        this.plugins[plugin_id].u_required_cf_version=stat.required_cf_version;
                    }
                    this.plugins={...this.plugins};
        ...
        doUpdate:async function(file,plugin_id){
            this.msg="";
            this.err="";
            if(this.plugins[plugin_id].u_required_cf_version !==undefined && this.cf_version<this.plugins[plugin_id].u_required_cf_version)
            {
            ...
            this.ajax_loading=true;
            await this.doDelayToUpdate();
            this.request.postRequestCb('req.php',{manage_plugins: 1,update_plugin: file,plugin_id},res=>{
                ...
                if(res=='1')
                {
                    ...
                    this.plugins[plugin_id].version=this.plugins[plugin_id].u_available_version;
                    this.plugins[plugin_id].u_stat_class="fa-check-circle text-success";
        ...
        doUpdateInFrontend:function(plugin){
            ...
            if(plugins[plugin] !==undefined && plugins[plugin].u_file !==undefined && plugins[plugin].u_stat=='available')
            {
                this.doUpdate(plugins[plugin].u_file,plugin);
            }
            if(plugins[plugin] !==undefined && plugins[plugin].u_stat!==undefined && plugins[plugin].u_stat=='none')
            {
        ...
        createUpdatePopup:function(plugin){
            let plugins=this.plugins;
            if(plugins[plugin] !==undefined && plugins[plugin].u_stat !==undefined)
            {
