...
$action="<table class='actionedittable'><tr><td><button class='btn unstyled-button edtbtn".$r->id."' data-toggle='tooltip' title='".t('Edit Product Detail')."' v-on:click='openEditor(\"".$detailjsn."\",0)'><i class='fas fa-edit text-primary'></i></button></td><td><a href='index.php?page=sales&product_id=".$r->id."'><button class='btn unstyled-button' data-toggle='tooltip' title='".t('Sales')."'><i class='fas fa-funnel-dollar text-success'></i></button></a></td><td><form action='' method='post' onsubmit=\"return confirmDeletion(".$count_product_used.",'product')\"><input type='hidden' name='delproduct' value='".$r->id."'><button type='submit' class='btn unstyled-button' data-toggle='tooltip' title='".t('Delete Product')."'><i class='fas fa-trash text-danger'></i></button></form></td></tr></table>";


//$totalsales=$data_arr['sell']->countProductSales($r->id);

echo "<tr><td>".t($hashcount)."</td><td>".$title."</td><td>".$r->productid."</td><td>".t(number_format($r->price))."</td><td>".$r->currency."</td><td><a href='index.php?page=sales&product_id=".$r->id."'>".t(number_format($r->sales_count))."</a></td><td>".date('d-M-Y h:ia',$r->createdon)."</td><td>".$action."</td></tr>";
}
}
?>
<tr><td colspan=10 class="total-data"><?php w('Total Products'); ?>: <?php
	if($products !==0)
	{ 
		echo t(number_format($products['total'])); 
	}else{echo 0;}
?></td></tr>
<!-- /keyword search -->
</tbody>
...
<style>.tmpltbdydiv .input-group{margin-top:2px;margin-bottom:2px;}</style>
