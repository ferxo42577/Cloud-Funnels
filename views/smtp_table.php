...
{
          ++$hashcount;
          $action="<table class='actionedittable'><tr><td><a href='index.php?page=smtp_create&id=".$res['id']."'><button class='btn unstyled-button' style='' data-toggle='tooltip' title='".t('Edit')."'><i class='fas fa-edit text-primary'></i></button></a></td><td><button class='btn unstyled-button' data-toggle='tooltip' title='".t("Test This SMTP")."' onclick=testSMTP(".$res['id'].",'".base64_encode($res['title'])."')><i class='fas fa-eye text-info'></i></button></td><td><form action='' method='post' onsubmit='return confirmDeletionSMTP(".$res['usedby_sequence'].",".$res['usedby_funnel'].");'><button type='submit' class='btn unstyled-button' value='".$res['id']."' name='userid' data-toggle='tooltip' title='".t("Delete SMTP")."'><i class='fas fa-trash text-danger'></i></button></form></td></tr></table>";
        ...
        echo "<tr>
          ...
          <td>".$res['hostname']."</td>
          <td>".$res['username']."</td>
          <td>".date('d-M-Y h:ia',$res['created_at'])."</td>
          <td>".$action."</td>
        </tr>";
      }
     ?>
    <tr><td colspan=10 class='total-data'><?php w("Total SMTPs"); ?>: <?php echo t($total_ob->countid); ?></td></tr>
<!-- /keyword search -->    
</tbody>     
    </table>
    </div>
...
function testSMTP(id,title)
{
  ...
  var smtpivclose=function(){document.getElementById("smtpcontainer").removeChild(div);dbgcontainer=0;};

  doEscapePopup(smtpivclose);
  document.getElementById("closesmtptestdiv").onclick=function(){smtpivclose()};
  dbgcontainer=div;
  ...
  div.getElementsByTagName("button")[0].onclick=function()
  {
   ...
   var debug=div.getElementsByTagName("select")[0].value;
   var inputs=div.getElementsByTagName("input");
   var reqstr={"qmlrtestsmtp":1,"toname":inputs[0].value,"toemail":inputs[1].value,"emailsubject":inputs[2].value,"emailbody":inputs[3].value,"debug":debug,"smtpid":id};
   request.postRequestCb('req.php',reqstr,function(data){
  data=data.trim();
  
  if(data=='1'){errspan.innerHTML="<font color='green'>"+t("Mail Sent Successfully")+"</font>";}
  else if(data=='0'){errspan.innerHTML="<font style='color:#e6005c;'>"+t("Unable To Send Mail")+"</font>";}
  else
  {
...
function confirmDeletionSMTP(bysequence,byfunnel)
{
...
<style type="text/css">
.actionedittable,.actionedittable tr,.actionedittable td
{
