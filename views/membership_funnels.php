...
<thead><th>#</th><th><?php w('Title'); ?></th><th><?php w('URL'); ?></th><th><?php w('Total&nbsp;Members'); ?></th><th><?php w('Created&nbsp;On'); ?></th><th><?php w('Action'); ?></th></thead>
...
++$hashcount;
++$count;
//$funneldata=$funnel->getFunnel($r->funnel_id,$select="`name`,`baseurl`");
$sumdata=$funnel->totalSumCountsFunelPages($r->funnel_id);
$action="<table class='actionedittable'><tr><td><a href='index.php?page=members&funnelid=".$r->funnel_id."'><button class='btn unstyled-button' data-toggle='tooltip' title='".t('View Members')."'><i class='fas fa-user-shield text-success'></i></button></a></td><td><a href='index.php?page=create_funnel&id=".$r->funnel_id."'><button class='btn unstyled-button' data-toggle='tooltip' title='".t('Edit Funnel')."'><i class='fas fa-edit text-primary'></i></button></a></td><td><form action='' method='post' onsubmit=\"return confirmDeletion('".$count_used_in."','".$count_used_type."')\"><input type='hidden' name='delfunnel' value='".$r->funnel_id."'><button type='submit' class='btn unstyled-button' data-toggle='tooltip' title='".t('Delete Funnel')."'><i class='fas fa-trash text-danger'></i></button></form></td></tr></table>";

$totalmembers=$r->count_members;

echo "<tr><td>".t($hashcount)."</td><td>".$r->funnel_name."</td><td><a href='".$r->funnel_baseurl."' target='_BLANK'>".$r->funnel_baseurl."</a></td><td><a href='index.php?page=members&funnelid=".$r->funnel_id."'>".t(number_format($totalmembers))."</a></td><td>".date('d-M-Y h:ia',$r->funnelcreatedon)."</td><td>".$action."</td></tr>";
}
}
?>
<tr><td colspan=10 class="total-data"><?php w('Total Projects') ?>: <?php echo t(number_format($datas['total_rows'])); ?></td></tr>
<!-- /keyword search -->
</tbody>
</table>
</div>
