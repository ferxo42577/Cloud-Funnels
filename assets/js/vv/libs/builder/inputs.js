...
var ImageInput = $.extend({}, Input, {
	...
	onUpload: function(event, node) {
		...
		function imageIsLoaded(e) {
				...
				for(let i in cf_global_img_upload_info)
				{
					formData.append(i,cf_request.escHTML(cf_global_img_upload_info[i]));
				}
				$.ajax({
					type: "POST",
					url: 'req.php',//set your server side upload script url
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
						...
						try{
							...
							data=data.src;
						}catch(err){console.log(err); uploaded=false;}
						if(!uploaded)
						{
