...
class Library
{
	...
	function loadJSTranslator($relative=true)
	{
		...
		{
			global $gcp_bucket;
			//if(!is_dir($gcp_bucket) && $self_hosted_gcp){mkdir($gcp_bucket);}
		...
		if($is_gcp && $gcp_bucket !==false)
		{
			$relative_cache_location= '<script src="'.$gcp_bucket_url.'/lang/cache.js?v='.$setup_token.'"></script>';
		}
	...
	function loadScript($str)
	{
		...
		return $bs."<script src='assets/js/".$str.".js?version=".$this->app_version."'></script>";
	}
...
<script type="text/javascript" href="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.8.0/Chart.min.js"></script>'.$chart;
	}
	function loadUser()
	{
		//load user data
		...
		//self::setInfo('valid_purchase',$ob->userDataIsValid(0));
		return $ob;
	}
	function loadFunnel()
	{
		...
		function loadAutoresponder()
	{
		...
		require_once("auto_responder.php");
		$infodata=self::getInfo();
		$data=new Autoresponder($infodata);
		return $data;
	}
	function loadOptin()
	{
	...
	function loadZapier()
	{
		require_once("zapier_integration.php");
		self::setInfo('load',$this);
		$ob=new Zapier_integration(self::getInfo());
        return $ob;
	}
	function cloneURL()
	{
