(function(){
    //------------------------------------------------
        ...
        <div class="col-sm-12 text-right">
            <button class="btn btn-primary theme-button" v-on:click="changeView('all')"><i class="fas fa-th-large"></i>&nbsp;View Files</button>
        </div>
        <div class="col-sm-12 uploader">
            ...
            <input type="file" vid="cf_file_inp" v-on:change="doUpload('change', $event)" style="display:none;"/></center>
            
            <center><span class="file-name" v-text="(storage.state.uploadable_file ===null)? 'No File Selected':storage.state.uploadable_file.name"></span></center>
            <center v-if="!uploading && (storage.state.uploadable_file !==null)"><button class="btn btn-primary theme-button" v-on:click="doUpload('upload')"><i class="fas fa-cloud-upload-alt"></i>&nbsp;Upload Now</button></center>
    ...
    let all_media=`<div class="row">
        ...
        <div class="row">
            <div v-bind:class="'col-sm-'+((current_file===null)? '12':'8')">
                ...
                <h1 class="text-center" style="opacity: 0.5">No file available, let's <span class="text-primary" v-on:click="changeView('uploader')" style="cursor: pointer;">upload</span> one<span v-if="getObjectKeyCount(storage.state.files.all)"> or <span class="text-primary" style="cursor:pointer;" v-on:click="doChangeCategory('all')">view all</span></span></h1>
                </div>
                <div class="col-sm-12" v-else>
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="input-group">
                                <div class="input-group-prepend"><span class="input-group-text">Select Type</span></div>
                        ...
                        <div class="col-sm-4">
                        <div class="input-group">
                            <div class="input-group-prepend"><span class="input-group-text">Arrange By</span></div>
                        ...
                        <div class="col-sm-4">
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Enter file name search" v-model="search_content">
                    ...
                    <div v-bind:class="['col-lg-'+((current_file===null)? '2': '3'), 'col-sm-4', 'each-mini-file', ((storage.state.selected_file !==null && storage.state.selected_file==file)? 'active': '')]" v-for="(data,file) in storage.state.files[storage.state.current_type]" v-on:click="doSelectFile(file)" v-if="storage.state.search_files.length<1 || storage.state.search_files.indexOf(file)>=1">
                        ...
                        <p class="mt-0">{{(data.title.trim().length<=25)? data.title.trim():(data.title.trim().substr(0, 25))+'...'}}</p>
                    </div>
                    <div class="col-sm-12" v-if="storage.state.search_files.length===1">
                        <h1 class="text-center" style="opacity: 0.5;">Nothing Found</h1>
                    </div>
                    </div>
                </div>
            </div>
            <div class="col-sm-4 h-100" v-if="current_file !==null">
                <div class="card h-100">
                    <div class="card-body h-100" style="height: 100%;">
                        ...
                        <video width="320" height="240" controls v-else-if="current_file.type=='video'">
                        ...
                        <center><span style="font-weight: 600;">{{current_file.title}}</span>&nbsp;&nbsp;<button class="btn unstyled-button mt-1" v-on:click="doDeleteFile()"><i class="fas fa-trash text-danger"></i></button></center>
                        <div class="mt-3 mb-4">
                        <label>Size: {{getSize(current_file.size)}}</label><br>
                        <label>MIME Type: {{current_file.file_type}}</label><br>
                        <label>Added On: {{current_file.added_on}}</label>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea 
                            id="cf_media_file_desc" class="form-control" v-text="current_file.description" placeholder="Enter file detail."></textarea>
                            </div>
                            <div class="form-group mt-4">
                            <div class="row">
                            <div class="col-sm-4"><button class="btn unstyled-button mt-1" v-on:click="doDeleteFile()"><i class="fas fa-trash text-danger"></i></button></div>
                            <div class="col-sm-8">
                            <button class="btn btn-primary theme-button btn-block" v-on:click="doSaveFileData()"><i class="fas fa-save"></i>&nbsp;&nbsp;Save Detail</button>
                            </div>
                            </div>
                            </div>
    ...
    <div class="container-fluid external-media-loader overlay">
    ...
    <div class="card-footer text-right" v-if="current_file !==null">
        <button class="btn theme-button" v-on:click="doExport()"><i class="fas fa-file-import"></i>&nbsp;&nbsp;Import</button>
    </div>
    </div>
    </div>
    </div>
    ...
    Vue.component('media_app',{
        template: `<div>
        <div v-if="use_mode=='external'">${external_container}</div>
        ...
        methods: {
            doUpload:function(step='init', e=null){
                ...
                if(step=='init')
                {
                    document.querySelectorAll(`input[vid='cf_file_inp']`)[0].click();
                }
                else if(step=='change')
                {
                ...
                else if(step=='upload')
                {
                    ...
                    this.request.postRequestCb('req.php',{
                        manage_media: 1,
                        upload: 1,
                        file: this.storage.state.uploadable_file
                    },(res)=>{
                        res=res.trim();
                        ...
                        {
                            this.err= "Unable to upload the file";
                            this.upload_stat= false;
                            console.log(res);
                        }
                        setTimeout(()=>{this.upload_stat= null;}, 1000);
                    });
                }
            },
            ...
            getCurrentFilesCount: function()
            {
                try
                {
                    return Object.keys(this.storage.state.files[this.storage.state.current_type]).length;
                }catch(err){return 0;}
            },
            changeView: function(view, re_render=false)
            {
                this.err="";
                this.msg="";
                this.storage.commit('update', {view,
                    selected_file: null,
            ...
            loadCurrentCategoryAssets: async function(init=false, page_only=false){
                    ...
                    try
                    {
                       ...
                       // console.log(res);
                        ...
                        if(init)
                        {
                            ...
                            /*data_to_update.current_type='all';*/
                        }
            ...
            doCopyCurrentURL:function(){
                copyText(this.storage.state.media_url+'/'+this.current_file.file);
            },
            ...
            doDeleteFile: function()
            {
                ...
                let file=this.current_file.file;
                let files=this.storage.state.files;
                types.forEach(type=>{
                    if(files[type][file] !==undefined)
                    delete files[type][file];
                });
                this.storage.commit('update', {selected_file: null,
                files,
                });
                this.request.postRequestCb('req.php', {manage_media:1, del_asset: file},(res)=>{
                    res=res.trim();
                });
            },
            getSize(bytes)
            ...
            doExport:function()
            {
                let is_html= ((cf_media_export_html !==undefined) && cf_media_export_html)? true: false;
                let current_file= this.current_file;

                if(this.current_file !==null)
                {
                    if(global_cf_media_export_callback !==undefined && global_cf_media_export_callback)
                    {
            ...
            doLoadMore: async function(e){
                ...
                let current_files_count= this.getObjectKeyCount(this.storage.state.files.all);
                if(current_files_count== files_count)
                {
                    this.page_load_err= `No more files available for this category`;
                    if(next>1)
                    {
                        this.storage.commit('update', {
            ...
            doSaveFileData(e)
            {
                this.err="";
                this.msg="";
                let title= document.querySelectorAll("#cf_media_file_title")[0].value.trim();

                let description= document.querySelectorAll("#cf_media_file_desc")[0].value.trim();
                if(title.length<1)
                {
