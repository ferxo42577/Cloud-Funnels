				...
				for (i in components)
				{
					...
					if (component)
					{
						...
						if (component.image) {
							...
							item.css({
								backgroundImage: "url(" + 'assets/js/vv/libs/builder/' + component.image + ")",
				...
				for (i in blocks)
				{
					...
					if (block)
					{
						...
						if (block.image) {
							...
							item.css({
								backgroundImage: "url(" + ((block.image.indexOf('//') == -1) ? 'assets/js/vv/libs/builder/':'') + block.image + ")",
								backgroundRepeat: "no-repeat"
							})
						}
	...
	_loadIframe : function(url) {	
	    ...
	    return this.documentFrame.on("load", function() 
        {
				...
				$(window.FrameWindow).on( "beforeunload", function(event) {
					...
					{
						var dialogText = t("You have unsaved changes");
	...
	saveAjax: function(fileName, startTemplateUrl, callback)
	{
		...
		try
		{
			try
			{
				...
				try
				{
					headers.querySelectorAll("script[cfdefaultscript='1']").forEach(data=>{
		...
		cf_html=cf_html.replace(/((true|false)\/)+(text\/javascript)/ig,`text/javascript`);
		...
		if(Array.isArray(matchesbg))
		{
			matchesbg.forEach(matchesbg_each=>{
				cf_html=cf_html.replace(matchesbg_each,matchesbg_each.replace(/&quot;/g,"'"));
			});
		}
		...
		config_alert.innerHTML="<h3><img src='assets/img/visual_cog.gif' style='max-width:100px;margin-left:8px;'>"+t("Saving Changes...")+"</h3>";
		document.body.appendChild(config_alert);

		cf_request.setCookie('templateedited_'+cf_page_fid+'_'+cf_page_lbl,cf_page_lbl,30);
		...
		$.ajax({
			...
			success: function (data) {
				...
				setTimeout(function(){
					cf_request.postRequestCb("req.php",{take_funnel_screenshot:1,funnel:cf_page_fid,page:cf_page_folder,abtype:cf_page_abtype,lavel:cf_page_lbl,category:cf_page_categ},(res)=>{
					  ...
					  var savealert=document.createElement("div");
						  ...
						  savealert.innerHTML="<h3><img src='assets/img/visual_done.gif?id="+Math.random()+"' style='max-width:100px;margin-left:8px;'>"+t("Saved Changes")+"</h3>";
...
Vvveb.Gui = {
	...
	download : function () {
		...
		try
		{
			filename=`${url.searchParams.get('fid')}-${url.searchParams.get('folder')}.html`;
		}catch(err){}
		//cf_installation_url
		...
		content_html=content_html.replace(/assets\/bootstrap/g,`${cf_installation_url}/assets/bootstrap`);
		content_html=content_html.replace(/assets\/js/g,`${cf_installation_url}/assets/js`);

		uriContent = "data:application/octet-stream,"  + encodeURIComponent(content_html);
		...
		function drawComponentsTree(tree)
		{
			...
			for (i in tree)
			{
				...
				if (tree[i].children.length > 0) 
				{
					...
					<label for="id' + j + '" style="background-image:url(assets/js/vv/libs/builder/' + node.image + ')"><span>' + node.name + '</span></label>\
