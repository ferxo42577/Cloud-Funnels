...
class CreateList{
	...
	elseif($_POST['createlist'] == 2)
	{
		...
		// echo $rows;
		if ($rows == 0) {
		$insertall = "insert into `".$pref."quick_email_lists` (`listid`, `name`, `email`, `exf`, `ipaddr`, `date`) values('".$listid."','".$name."','".$email."','','".$ipaddress."','".$date."')";
		$result = $mysqli->query($insertall);
		return 1;
	}
	else{
	    return 0;
