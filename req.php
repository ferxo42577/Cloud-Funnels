...
{
	//config ajax
	if((isset($_POST['token'])&& $security->matchToken($_POST['token']))|| isset($gcp_connecter))
	{
	if(!is_file($GLOBALS["config_file"]))
	{
		$host=$_POST['host'];
...
if(isset($_POST['chnglbl']))
{
	...
	if($_POST['lbls']=='html')
	{
		$funnel->changeLabel($_POST['funnel_id'],array(),$_POST['lblhtml']);
	}
	else
	{
		$lbls=json_decode($_POST['lbls']);
		$funnel->changeLabel($_POST['funnel_id'],$lbls,$_POST['lblhtml']);
	}
}
//-------delete label
...
if (isset($_POST['mailengine']) || isset($_POST['activecampaignn']) || isset($_POST['mailchimp']) || isset($_POST['getresponse']) || isset($_POST['constantcont']) || isset($_POST['ontraport']) || isset($_POST['hubspot']) || isset($_POST['aweber'])|| isset($_POST['sendiio']) || isset($_POST['mymailit']) || isset($_POST['mailwizz']) || isset($_POST['moosend']) || isset($_POST['mautic']) || isset($_POST['mailerlite'])) {
	if(!$userobforcheck->isLoggedin()){die ('@not-logged-in@');}
	$autores = $load->loadAutoresponder();
	$saveSettings = $autores->saveApiDetails();
	if ($saveSettings == 1) {
		echo 1;
	}            
	else{
