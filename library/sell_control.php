...
class Sellcontrol
{
	...
	function createProduct($product,$title,$description,$url,$price,$currency,$sheeping,$subproducts,$opproducts,$tax,$doupdate=0)
	{
		$plugin_loader=false;
		...
		$in=$mysqli->query("insert into `".$table."` (`productid`,`title`,`url`,`description`,`price`,`currency`,`shipping`,`subproducts`,`opproducts`,`tax`,`createdon`) values ('".$product."','".$title."','".$url."','".$description."','".$price."','".$currency."','".$sheeping."','".$subproducts."','".$opproducts."','".$tax."','".time()."')");
		...
		$u=$mysqli->query("update `".$table."` set `productid`='".$product."',`title`='".$title."',`url`='".$url."',`description`='".$description."',`price`='".$price."',`currency`='".$currency."',`shipping`='".$sheeping."',`subproducts`='".$subproducts."',`opproducts`='".$opproducts."',`tax`='".$tax."' where `id`='".$doupdate."'");
	...
	function getProductForView($last=0)
	{
		...
		if(isset($_POST['onpage_search']) && strlen($_POST['onpage_search'])>0)
		{
			...
			$qry="select `a`.*,".$salescountquery." from `".$table."` as `a`  where `productid` like '%".$search."%' or `title` like '%".$search."%' or `url` like '%".$search."%' or `description` like '%".$search."%' order by `id` desc";
		}
		else
		{
		$order_by="`a`.id desc";
	...
	function doPayment($funnelid,$folder,$pageid,$abtype='a',$productid,$paymentmethod_id,$membership,$lists,$optionalproducts=array(),$confirm_url,$cancel_url,$userdata)
	{
		//echo "test";
		...
		{
		//do paypal payment
		require_once(__DIR__ ."/payment_apis/paypal/function.php");
		$done=addToPaypal($this,$payment_data->credentials,$productsarr,$product->description);

		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		$payer_data=$paymentdata->payer->payer_info;
		$payer_name=$payer_data->first_name." ".$payer_data->last_name;
		...
		{
		//do paypal payment
		require_once(__DIR__ ."/payment_apis/payu/function.php");
		$done=addToPayu($this,$payment_data->credentials,$productsarr,$product->description);
		...
		{
		//do paypal payment
		require_once(__DIR__ ."/payment_apis/toyyibpay/function.php");
		$done=addToToyyibpay($this,$payment_data->credentials,$productsarr,$product->description);
		...
		$paymentdata=json_decode($done);
			...
			$payment_id=$paymentdata->payer_id;
			}
		}
		elseif($method=="xendit")
		{
		//Xendit payment
		require_once(__DIR__ ."/payment_apis/xendit/function.php");
		$done=addToXendit($this,$payment_data->credentials,$productsarr,$product->description);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		{
			require_once(__DIR__ ."/payment_apis/midtrans/function.php");
			$done=Midtrans\addToMidtrans($this,$payment_data->credentials,$productsarr,$product->description);
		...
		{
			//razorpay payment
			require_once(__DIR__ ."/payment_apis/razorpay/function.php");
			$done=addToRazorpay($this,$payment_data->credentials,$productsarr,$product->description);
		...
		{
			require_once(__DIR__ ."/payment_apis/ccavenue/function.php");
			$done=addToCcavenue($this,$payment_data->credentials,$productsarr,$product->description);
		...
		{
		//stripe payment
		require_once(__DIR__ ."/payment_apis/stripe/function.php");
		$done=addToStripe($this,$payment_data->credentials,$productsarr,$product->description);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		require_once(__DIR__ ."/payment_apis/authorize.net/function.php");
		$done=addToAuthorizeDotNet($this,$payment_data->credentials,$productsarr,$product->description);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);

		$payer_name=$_SESSION['authorizedotnet_payer_name'.get_option('site_token')];
		$payer_email=$_SESSION['authorizedotnet_payer_email'.get_option('site_token')];
		$payment_id=$paymentdata->transId;
			}
		}
		elseif($method=="instamojo")
		{
		require_once(__DIR__ ."/payment_apis/instamojo/function.php");
		$done=addToInstamojo($this,$payment_data->credentials,$productsarr,$product->description);
		...
		elseif($method=="jvzoo_ipn")
		{
		//JVZoo Ipn Payment
		require_once(__DIR__ ."/payment_apis/ipns/jvzoo.php");
		$done=addToJvzoo($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		elseif($method=="warriorplus_ipn")
		{
		//warrior plus ipn payment
		require_once(__DIR__ ."/payment_apis/ipns/warriorplus.php");
		$done=addToWarriorPlus($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		elseif($method=="paydotcom_ipn")
		{
		//paydotcom ipn payment
		require_once(__DIR__ ."/payment_apis/ipns/paydotcom.php");
		$done=addToPayDotCom($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		elseif($method=="paykickstart_ipn")
		{
		//paykickstart plus ipn payment
		require_once(__DIR__ ."/payment_apis/ipns/paykickstart.php");
		$done=addToPaykickstart($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		elseif($method=="thrivecart_ipn")
		{
		//paykickstart plus ipn payment
		require_once(__DIR__ ."/payment_apis/ipns/thrivecart.php");
		$done=addToThriveCart($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
		$paymentdata=json_decode($done);
		...
		elseif($method=="clickbank_ipn")
		{
		//paykickstart plus ipn payment
		require_once(__DIR__ ."/payment_apis/ipns/clickbank.php");
		$done=addToClickbank($payment_data->credentials);
		if($done !==0 && (isset($_GET['execute'])))
			{
				$paymentdata=json_decode($done);
	...
	function checkOutDetailcreate($paymentmethod_id,$productid,$optionalproducts)
	{
		...
		//$method=$payment_data->method;
		//$credentials=json_decode($method);
	...
	function getProductsPriceDependingOnMethod($credentials,$items,$checkout_page=0)
	{
		...
		{
			return compact('itemarr','sheepingcharge','tax','totalprice','currency','total','allproductdetail','items');
		}
	}
	function productTemplatecreator($html,$productids)
	{
		$pregtestfordom="/(cf-loop(?=((=['\"]products)+)))+/";
