...
this.doDelayInPost=function(cb){
	try
	{
		//console.log(global_req_is_gcp);console.log("-----");
		if(global_post_requests.length<1 || global_req_is_gcp===undefined|| global_req_is_gcp===false)
		{
			//console.log("pot request granted");
			cb(1);
		}
		else
		{
			//console.log("pot request in hold");
			setTimeout((cb)=>{ this.doDelayInPost(cb);  },200,cb);
		}
	}
	catch(err)
	{
...
this.postRequestCb=async function(url,data,callback)
{
	...
	await new Promise((resolve,reject)=>{
		this.doDelayInPost(function(data){ resolve(1); });
	});
...
this.createCSRF=function(callback){
	this.postRequestCb('req.php',{"checkcsrf":"create"},function(data){callback(data);});
};
this.matchCSRF=function(token,callback){
	this.postRequestCb('req.php',{"checkcsrf":"match","token":token},function(data){
		callback(data.trim());
	});
}
...
this.openLoginScreen=function(data)
 {
	 ...
	 if(data.trim().length>0)
	 {
		 ...
		 if(open && !this.loginscreen_opened)
		 {
			...
			window.open("index.php?page=login&autologin=1", "_blank", "toolbar=yes,height=800,width=1600");
		 }
...
function confirmDeletion(count=0,type="Data")
{
...
jQuery(document).ready(function($){
	try
	{
		...
		$(".srchcontainer .btn").click(function(){
			if(open==1)
			{$(".datesearchformdata").show(500);open=0;oopen=1;}
			else {
	    ...
	    $(document).click(function(){
			try
			{
			if(oopen==0){$(".datesearchformdata").hide(500);};
			oopen=0;
			}catch(errr){}
		});	
		}
...
try
{
	jQuery(document).ready(function()
	{
	try{document.getElementById("historyback").onclick=function(){window.history.back();};}catch(errrr){}
	});
}catch(err)
{console.log(err);}
function OnPageSearch(search="",el=null)
{
	...
	try
	{
		...
		{
			window.req_onpage_search_parent=document.querySelectorAll(this.el)[0].innerHTML;
		}
	}
	catch(errrr)
	{
	...
	this.search=function(){
		...
		else if(window.req_onpage_search_parent !==undefined)
		{
			...
			if(this.do_modify)
			{
			document.querySelectorAll(this_class.el)[0].innerHTML=window.req_onpage_search_parent;
			}
			else
			{
...
document.onreadystatechange=function(){
	if(this.readyState=="complete")
	{
		try
		{
		document.querySelectorAll(".qfnl_max_records_per_page")[0].onchange=function(){
			document.getElementsByClassName("qfnl_max_records_per_page_btn")[0].click();
		}
		}catch(errr){}
		try
		{
			var totalDataReg=/([0-9,])+/g;
			var toalsatael_text=document.querySelectorAll("td.total-data")[0].innerText.trim();
			tataldatacount=toalsatael_text.match(totalDataReg)[0];
			//console.log(tataldatacount.replace(",","").trim());
			new ajaxRequest().postRequestCb("req.php",{"qfnl_current_page_maxdata":tataldatacount},function(data){});
		}
		catch(err)
		{
			console.log(err.message+"--");
...
else
{
	...
	var data={"chkforauthvalidationpucrhase":1,"auth_valid_user":email,"auth_valid_order_code":ordercode};
	request.postRequestCb('req.php',data,function(res){
		res=res.trim();
		if(res=='1')
		{
			errdoc.innerHTML=t("Successfully validated, redirecting you in a moment.");
